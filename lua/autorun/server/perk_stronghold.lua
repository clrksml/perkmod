if !string.find(string.lower(engine.ActiveGamemode()), "stronghold") then return end

/*
	The following of code is for stronghold.
*/
...
perkXPTime = 10 // The amount of xp to be given on every 5 minutes.
...
local lastSave = CurTime() + 300
hook.Add("Think", "perkThink", function(  )
	if lastSave <= CurTime() then
		for k, ply in pairs(player.GetAll()) do
			ply:perkAddXP(perkXPTime)
			ply:perkSaveData()
		end
		
		lastSave = CurTime() + 300
	end
end)
